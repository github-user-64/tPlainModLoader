...
{
    internal class Patch_ModRemadeChatMonitor
    {
        internal static List<ModRemadeChatMonitor> mod = new List<ModRemadeChatMonitor>();

        internal static void Initialize(IAddPatch addPatch)
        {
            Type oType = typeof(RemadeChatMonitor);
            Type tType = typeof(Patch_ModRemadeChatMonitor);
        ...
        public static void DrawChatPostfix(bool drawingPlayerChat)
        {
            try
            {
                foreach (ModRemadeChatMonitor item in mod) item.DrawChatPostfix(drawingPlayerChat);
            }
            catch (Exception ex)
            {
        ...
        public static void AddNewMessagePrefix(ref string text, Color color, int widthLimitInPixels = -1)
        {
            try
            {
                foreach (ModRemadeChatMonitor item in mod) item.AddNewMessagePrefix(ref text, color, widthLimitInPixels);
            }
            catch (Exception ex)
            {
                OutputDebug.OutputException(ex);
