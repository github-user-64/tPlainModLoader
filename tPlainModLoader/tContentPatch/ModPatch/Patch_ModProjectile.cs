...
{
    internal class Patch_ModProjectile
    {
        internal static List<ModProjectile> mod = new List<ModProjectile>();

        internal static void Initialize(IAddPatch addPatch)
        {
            Type oType = typeof(Projectile);
            Type tType = typeof(Patch_ModProjectile);
        ...
        public static void UpdatePrefix(Projectile __instance, int i)
        {
            try
            {
                foreach (ModProjectile item in mod) item.UpdatePrefix(__instance, i);
            }
            catch (Exception ex)
            {
        ...
        public static void UpdatePostfix(Projectile __instance, int i)
        {
            try
            {
                foreach (ModProjectile item in mod) item.UpdatePostfix(__instance, i);
            }
            catch (Exception ex)
            {
                OutputDebug.OutputException(ex);
        ...
        public static void KillPrefix(Projectile __instance)
        {
            try
            {
                foreach (ModProjectile item in mod) item.KillPrefix(__instance);
            }
            catch (Exception ex)
            {
                OutputDebug.OutputException(ex);
